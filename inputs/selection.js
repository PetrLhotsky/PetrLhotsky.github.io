...
function selectionW3(id) {
  ...
  x = [];
  x.push(document.getElementById(id));
  l = x.length;
  for (i = 0; i < l; i++) {
    ...
    for (j = 1; j < ll; j++) {
      ...
      c.addEventListener("click", function() {
        updateSelectBoxOnClick(this);  // Použití extrahované funkce
      });
      b.appendChild(c);
    }
    x[i].appendChild(b);
...
function updateSelectBoxOnClick(selectedElement) {
  var y, i, k, s, h, sl, yl;
